...
describe('refreshAuthToken', () => {
    it('missing refresh token throw error', async () => {
        await expect(auth.refreshAuthToken(<any>{})).rejects.toThrow(
            new BadCredentialsError(undefined, 'jwt must be provided'),
        );
    });
    it('malformed refresh token throw error', async () => {
        await expect(auth.refreshAuthToken({ refreshToken: 'xxx' })).rejects.toThrow(
            new BadCredentialsError(undefined, 'jwt malformed'),
        );
    });
});
