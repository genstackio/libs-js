...
export function useAppContext({storageKeys, themes, muiTheme, queries, translations, locales, defaultLocale = 'en-US', fallbackLocale = 'en-US', getImage}: app_context_params) {
    ...
    const locale = lang;
    const enrichedSetCart = useCallback(cart => {
    ...
    const enrichedSetUser = useCallback(user => {
    ...
    const setCurrentTokens = useCallback(async (tokens: any) => {
        ...
        await refreshUser();
    }, [enrichedSetUser, refreshUser]) as any;

    const onLogout = useCallback(async () => {
        await enrichedSetUser(undefined);
    }, [enrichedSetUser]) as any;

    const apiProviderValue: any = useMemo<{gql: Function, useMutation: Function, useQuery: Function, useLazyQuery: Function}>(() => ({gql, useMutation, useQuery, useLazyQuery}), []);
    const cartProviderValue: any = useMemo<{cart: cart|undefined, setCart: Function, resetCart: Function}>(() => ({cart: fetchCartFromLocalStorage(), setCart: enrichedSetCart, resetCart}), [cart, enrichedSetCart, resetCart]);
    const userProviderValue: any = useMemo<user_context_value>(() => ({user: fetchUserFromLocalStorage(), setUser: enrichedSetUser, getTokens: getCurrentTokens, setTokens: setCurrentTokens, logout: onLogout, refreshUser}), [fetchUserFromLocalStorage, user, enrichedSetUser, setCurrentTokens, getCurrentTokens, onLogout, refreshUser]);
    ...
    const themeFactory = useCallback((old: any = {}) => ({
        ...old,
        ...((themes as any)[theme] || {}),
    }), [themes, theme]);
