...
export function Avatar({image, name, size = 'md', shape = 'circular', status = undefined}: AvatarProps) {
    return (
        <div className={clsx('inline-block', status && 'relative')}>
            {!image && <div className={clsx(sizes[size], shapeClass(shape), 'text-white bg-gray-400 flex items-center justify-center')}>{getInitials(name)}</div>}
            {image && <img src={image.url} alt={image.alt} className={clsx(sizes[size], shapeClass(shape))} />}
            {status && <div className={clsx(statusClass(status), 'absolute bottom-0 right-0 w-1/4 h-1/4 rounded-full border-1 border-white')} />}
        </div>
    );
}
