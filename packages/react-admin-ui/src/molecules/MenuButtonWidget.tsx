    ...
    return (
        <ul className={'px-2 py-3 rounded-md max-w-xxs divide-y'}>
            {items.map(({icon, label, target}, index) => (
                <li className={clsx(textClass({color: color, variant: 'light', hoverable: true}), 'flex space-x-2 py-2 px-4 rounded-md cursor-pointer')} key={index} onClick={handleClick(target)}>
                    {icon && <Icon icon={icon} />}
