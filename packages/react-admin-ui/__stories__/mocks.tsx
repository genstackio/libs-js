export async function generic(data) {
    ...
    switch (data?.options?.variables?.data?.username || data?.options?.variables?.data?.email) {
        ...
        case 'blocked@email.com':
            result = {loading: false, error: {username: {message: 'Account is blocked'}, email: {message: 'Account is blocked'}}, data: undefined};
            break;
        case 'disabled@email.com':
            result = {loading: false, error: {username: {message: 'Account is disabled'}, email: {message: 'Account is disabled'}}, data: undefined};
            break;
        ...
        case 'timeout@email.com':
            result = {loading: false, error: {'': {message: 'There was a timeout error, please try again later'}}, data: undefined};
            duration = 5000;
            break;
        default:
