...
import {
    ...
    ReindexResponse,
    script,
    SearchAllIndicesInput,
    SearchAllIndicesResponse,
    SearchInput,
    SearchResponse,
    ShrinkIndexInput,
    ShrinkIndexResponse,
...
export class Sdk extends BaseSdk {
    ...
    async bulkActions(actions: bulk_action[] = [], def?: BulkActionsInput): Promise<BulkActionsResponse> {
        return this.request('_bulk', 'POST', {
            _rawBody: actions.map((a) => (undefined === a ? '' : JSON.stringify(a))).join('\n') + '\n',
            ...def,
        });
    }
    async indexBulkActions(
        name: string,
        actions: index_bulk_action[] = [],
        def?: IndexBulkActionsInput,
    ): Promise<IndexBulkActionsResponse> {
        return this.requestIndex(name, '_bulk', 'POST', {
            _rawBody: actions.map((a) => (undefined === a ? '' : JSON.stringify(a))).join('\n') + '\n',
            ...def,
        });
    }
    ...
    async getIndexDocuments(
        index: string,
        docs: index_document_query[],
        def?: GetIndexDocumentsInput,
    ): Promise<GetIndexDocumentsResponse> {
    ...
    async deleteIndexDocument(
        index: string,
        id: string,
        def?: DeleteIndexDocumentInput,
    ): Promise<DeleteIndexDocumentResponse> {
        return this.requestIndexDocument(index, id, '_doc', 'DELETE', def);
    }
    async updateIndexDocument(
        index: string,
        id: string,
        def: UpdateIndexDocumentInput,
    ): Promise<UpdateIndexDocumentResponse> {
        return this.requestIndexDocument(index, id, '_update', 'POST', def);
    }
    async updateIndexDocumentsByQuery(
        index: string,
        query: update_query,
        script: script,
        def?: UpdateIndexDocumentsByQueryInput,
    ): Promise<UpdateIndexDocumentsByQueryResponse> {
        return this.requestIndex(index, '_update_by_query', 'POST', { query, script, ...def });
    }
    async deleteIndexDocumentsByQuery(
        index: string,
        query: delete_query,
        def?: DeleteIndexDocumentsByQueryInput,
    ): Promise<DeleteIndexDocumentsByQueryResponse> {
    ...
    async searchIndexPage(
        name: string,
        query?: any,
        offset?: number,
        limit = 50,
        def?: Omit<SearchInput, 'query'>,
        sort?: string,
    ) {
    ...
    protected async findAllPages<T = any>(
        pageFetcher: (cursor: string) => Promise<{ items: T[]; cursor?: string; count?: number }>,
    ): Promise<T[]> {
