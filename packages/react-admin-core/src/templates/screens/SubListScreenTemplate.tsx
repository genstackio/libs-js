...
function SubListScreenTemplate({
    searchSwitch = false,
    deletable = true,
    moveable = true,
    search = false,
    ...
    const {
        ...
        pMode = undefined,
        pCursors = undefined,
    } = useParams<{ parentId: string; pPage?: string; pSize?: string; pMode?: string; pCursors?: string }>();
    ...
    const [upDoc] = useConditionalMutationApi(
        moveable,
        moveQueryName
            .replace('{parentName}', parentName)
            .replace('{parentId}', parentId)
    ...
    const [downDoc] = useConditionalMutationApi(
        moveable,
        moveQueryName
            .replace('{parentName}', parentName)
            .replace('{parentId}', parentId)
    ...
    const { data, loading, error, refetch } = useQueryApi(
        `${searchMode ? 'SEARCH' : 'FIND'}_${parentSingularName.toUpperCase()}_${name.toUpperCase()}`,
        {
    ...
    const handleDelete = useCallback(
        (id) => () => {
            ...
            p.then(() => {
                ...
                refetch({
                    variables: { id: parentId, offset: page.currentCursor, limit: page.size, ...fetchSortVariables },
                });
            });
    ...
    return (
        <>
            ...
            {!items.length && !loading && (
                <Alert color={'warning'}>
                    {t([`screen_${parentSingularName}_${name}_list_table_empty`, 'screen_generic_list_table_empty'])}
                </Alert>
            )}
